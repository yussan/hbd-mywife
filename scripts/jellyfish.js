...
function mouseMove(hoverX, hoverY, objects) {
    if (!window.jellyfishReady) return; // Disable mouse movement until intro animation finishes
...
for (let i = 0; i < objects.length; i++) {
    objects[i].addEventListener('click', () => {
        console.log(text.length); // fixed typo lenght -> length
        for (let j = 0; j < 30; j++) {
            let bub = [];
        ...
        popSound1.play().catch(e => console.log(e));
        popSound2.play().catch(e => console.log(e));
    });
}
